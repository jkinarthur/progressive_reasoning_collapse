...
\begin{enumerate}
    ...
    \item[\textbf{C8.}] We demonstrate that \textbf{CARR scales to pre-trained LLM architectures} (T5-Large, 770M parameters), confirming that the theoretical framework and compression mechanism generalise from standard transformer backbones to large-scale language models.
    \item[\textbf{C9.}] We provide \textbf{robustness analysis} under long-context histories, noisy prompts, and cross-domain transfer, and a systematic \textbf{failure case analysis} identifying conditions where collapse-aware compression provides limited benefit.
\end{enumerate}
...
Neural collapse~\cite{papyan2020prevalence} describes the phenomenon where class means converge to a simplex equiangular tight frame (ETF) during the terminal phase of training. This has been extended to understand feature learning dynamics~\cite{zhu2021geometric,han2022neural}.

It is important to clearly distinguish \emph{neural collapse} from \emph{Progressive Reasoning Collapse (PRC)}. Neural collapse is a \emph{training-time} phenomenon: class means organise into a regular geometric structure as the classifier converges to its optimum. PRC is an \emph{inference-time} phenomenon: it describes how the latent multi-intent geometry of a \emph{fully trained} recommender degenerates when the model is compressed during inference. Neural collapse strengthens class discriminability in single-label classification; PRC destroys intent diversity in multi-intent recommendation. Geometrically, neural collapse forms a simplex ETF among a fixed set of class means, whereas PRC causes multiple intent modes to collapse toward a \emph{single} dominant latent mode, eliminating the diversity required for faithful recommendation reasoning. Our work draws on the geometric intuition of the neural collapse literature to formalise a new and distinct failure mode.
...
\subsection{Modern LLM-Based Recommendation Methods}
A growing body of work develops LLM-based recommenders using modern architectures and training paradigms. LLMRec~\cite{wei2024llmrec} augments collaborative filtering with LLM-generated semantic user and item profiles. UniSRec~\cite{hou2022unisrec} employs universal sequence representations transferable across domains using pre-trained language models. PromptRec~\cite{liu2023pre} adapts prompt engineering for zero-shot item ranking. RecGPT~\cite{liao2024recgpt} fine-tunes GPT-style models for personalised sequential generation. These methods demonstrate that LLM capabilities substantially improve recommendation quality when properly harnessed. Our CARR framework is complementary and orthogonal to these advances: it addresses the \emph{compression and reasoning-preservation} problem that arises whenever any such model is accelerated for large-scale deployment. We include direct experimental comparisons against LLMRec and UniSRec to establish CARR's effectiveness relative to the modern LLM recommendation literature.
...
We compare against three groups of methods:

\textbf{Compression baselines:} \textbf{Full-LLM} (uncompressed), \textbf{Fixed-Early/Mid/Late} (fixed compression at layers 3/6/9), \textbf{Prompt-Pruned} (prompt truncation~\cite{jiang2023longllmlingua}), \textbf{Layer-Skipping} (late-layer simplification), and \textbf{KV-Pruning} (heavy-hitter KV cache eviction~\cite{zhang2024h2o}).

\textbf{Modern LLM recommendation:} \textbf{LLMRec}~\cite{wei2024llmrec} (LLM-augmented collaborative filtering) and \textbf{UniSRec}~\cite{hou2022unisrec} (universal sequence representation with pre-trained LM).

\textbf{Neural sequential baselines:} \textbf{SASRec}~\cite{kang2018sasrec}, \textbf{BERT4Rec}~\cite{sun2019bert4rec}, and \textbf{GRU4Rec}~\cite{hidasi2016gru4rec}.
...
\begin{table}[!t]
\caption{Cluster Semantic Alignment on ML-1M, Layer 12\label{tab:cluster}}
\centering
\begin{tabular}{l|ccc}
\toprule
\textbf{Method} & Purity & NMI & ARI \\
\midrule
Full-LLM & 0.694 & 0.521 & 0.448 \\
Fixed-Early ($k=3$) & 0.412 & 0.186 & 0.112 \\
Fixed-Mid ($k=6$) & 0.538 & 0.324 & 0.251 \\
Fixed-Late ($k=9$) & 0.621 & 0.437 & 0.368 \\
\textbf{CARR} & \textbf{0.671} & \textbf{0.498} & \textbf{0.418} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:cluster} confirms that Full-LLM latent clusters strongly align with item categories (NMI $= 0.521$), validating that our multi-intent geometry captures semantically meaningful recommendation intent modes rather than arbitrary geometric structure. Fixed-Early compression severely disrupts this alignment (NMI $= 0.186$, a 64\% drop), while CARR maintains high alignment (NMI $= 0.498$). This directly establishes that reasoning-collapse scores track genuine semantic degradation.

\subsubsection{Intent Perturbation Test}
To further validate that clusters encode distinct reasoning modes, we remove representations associated with a specific cluster and measure degradation in HR@10. If clusters encode recommendation-relevant intent modes, targeted removal should cause significantly larger degradation than random removal of an equal-sized subset.

\begin{table}[!t]
\caption{Intent Perturbation Test on ML-1M: HR@10 Reduction from Baseline\label{tab:perturbation}}
\centering
\begin{tabular}{l|cc}
\toprule
\textbf{Removal Type} & $\Delta$HR@10 & $p$-value \\
\midrule
Random subset & $-0.008 \pm 0.004$ & $0.31$ \\
Intent cluster (majority) & $-0.041 \pm 0.006$ & $<$0.001 \\
Intent cluster (minority) & $-0.028 \pm 0.007$ & $<$0.01 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:perturbation} confirms that intent-cluster removal causes significantly larger degradation than random removal ($p < 0.001$), establishing that the geometric clusters encode genuine recommendation reasoning modes.

\subsection{Experiment 1b: Empirical Validation of Contractive Dynamics}
Theorem~1 relies on compressed transformer layers approximating contractive linear operators (Assumption~1). To validate this, we estimate the Jacobian matrix $J_l$ of each compressed layer using automatic differentiation and compute its spectral norm $\|J_l\|_2$ via power iteration. A layer is contractive if $\|J_l\|_2 < 1$.

\begin{table}[!t]
\caption{Jacobian Spectral Norms of Compressed Layers on ML-1M\label{tab:jacobian}}
\centering
\begin{tabular}{l|cccc}
\toprule
\textbf{Layer} & $l=4$ & $l=6$ & $l=8$ & $l=10$ \\
\midrule
$\|J_l\|_2$ (uncompressed) & 0.982 & 0.971 & 0.965 & 0.958 \\
$\|J_l\|_2$ (after compression) & 0.831 & 0.754 & 0.712 & 0.688 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:jacobian} shows that all compressed layers satisfy $\|J_l\|_2 < 1$, confirming contraction. The spectral norm decreases monotonically with depth, consistent with the progressive collapse predicted by Theorem~1. This provides direct empirical support for the linearised contractive propagation model and addresses the concern that transformers are highly nonlinear.
...
\midrule
LLMRec & 0.398 & 0.261 & 0.305 & 0.193 & 0.278 & 0.171 & 0.348 & 0.222 & 0.92$\times$ & 39.1ms \\
UniSRec & 0.381 & 0.248 & 0.291 & 0.184 & 0.264 & 0.161 & 0.332 & 0.211 & 0.78$\times$ & 33.2ms \\
KV-Pruning & 0.372 & 0.241 & 0.275 & 0.173 & 0.251 & 0.153 & 0.318 & 0.200 & 0.61$\times$ & 26.1ms \\
\midrule
...
Table~\ref{tab:main} shows that CARR achieves the best trade-off between quality and efficiency. Compared to Full-LLM, CARR reduces FLOPs by 45\% while maintaining 96\% of HR@10. Compared to Fixed-Mid with similar efficiency, CARR improves HR@10 by 13\% relative. Importantly, CARR outperforms the modern LLM-based baselines LLMRec and UniSRec on all four datasets despite achieving significantly lower latency (23.5ms vs.\ 39.1ms and 33.2ms), demonstrating that collapse-aware compression provides both quality and efficiency advantages over methods that simply augment LLMs with richer information sources.
...
To analyse the geometric structure of the collapse phenomenon, we visualise latent representations at layer 12 using three complementary dimensionality reduction methods: t-SNE, PCA, and UMAP. Using multiple methods guards against artefacts specific to any single technique.

\begin{figure}[!t]
\centering
\includegraphics[width=3.2in]{figures/tsne_placeholder}
\caption{Latent representation visualisations at layer 12 on ML-1M (t-SNE, PCA, and UMAP shown per method). (a) Full-LLM: distinct intent clusters preserved across all three projections. (b) Fixed-Mid: collapsed to a single cluster. (c) CARR: clusters maintained with compression.}
\label{fig:tsne}
\end{figure}

\begin{table}[!t]
\caption{Cluster Statistics at Layer 12 on ML-1M\label{tab:cluster_stats}}
\centering
\begin{tabular}{l|ccc}
\toprule
\textbf{Method} & Num.\ Clusters & Inter-cluster Dist. & Intra-cluster Var. \\
\midrule
Full-LLM & 4.8 & 1.241 & 0.318 \\
Fixed-Mid & 1.9 & 0.412 & 0.084 \\
\textbf{CARR} & 4.2 & 1.089 & 0.297 \\
\bottomrule
\end{tabular}
\end{table}

Full-LLM maintains distinct intent clusters through all layers (Table~\ref{tab:cluster_stats}: mean 4.8 clusters, high inter-cluster distance). Fixed-Mid collapses to near a single cluster after layer 6 (1.9 effective clusters). CARR preserves cluster separation while achieving compression, with inter-cluster distance 87\% of Full-LLM's versus 33\% for Fixed-Mid.

\subsection{Experiment 7: Scalability to Pre-Trained LLM Architectures}

To validate that CARR generalises beyond the 12-layer backbone, we apply the CARR compression mechanism to T5-Large (770M parameters, 24 layers, $d=1024$)~\cite{raffel2020t5}, fine-tuned for sequential recommendation on ML-1M.

\begin{table}[!t]
\caption{CARR with T5-Large Backbone on ML-1M\label{tab:t5}}
\centering
\begin{tabular}{l|cc|cc}
\toprule
\textbf{Method} & HR@10 & NDCG & FLOPs & Latency \\
\midrule
T5-Large (Full) & 0.441 & 0.289 & 1.00$\times$ & 128.4ms \\
T5-Large + Fixed-Mid & 0.374 & 0.241 & 0.51$\times$ & 66.2ms \\
\textbf{T5-Large + CARR} & \textbf{0.424} & \textbf{0.275} & 0.58$\times$ & 74.8ms \\
\midrule
$\Delta$ (CARR vs.\ Fixed-Mid) & $+$13.4\% & $+$14.1\% & -- & -- \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:t5} demonstrates that CARR generalises to T5-Large with consistent relative improvements: 13.4\% HR@10 gain over fixed compression, matching the 13.1\% improvement on the 12-layer backbone. This confirms that the theoretical framework and compression mechanism scale across backbone sizes.

\subsection{Experiment 8: Robustness Analysis}

We evaluate CARR under three stress conditions to assess robustness.

\subsubsection{Long-Context Histories}
For users with history length $|H| \in [100, 200]$ (top quartile of ML-1M), collapse risk is elevated as longer histories contain more evidence to preserve.
...
\midrule
Fixed-Mid & 0.298 & 0.189 \\
\textbf{CARR} & \textbf{0.368} & \textbf{0.237} \\
$\Delta$ & $+$23.5\% & $+$25.4\% \\
\bottomrule
\end{tabular}
\end{table}

CARR shows larger relative gains on long-context users (23.5\%) than on the average-length population (13.1\%), consistent with Theorem~2: the greater the evidence in the history, the more damaging unconstrained collapse.

\subsubsection{Noise Injection}
We inject Gaussian noise ($\sigma \in \{0.1, 0.5, 1.0\}$) into prompt embeddings to simulate corrupted or incomplete interaction histories.

\begin{table}[!t]
\caption{Robustness to Prompt Noise (ML-1M, HR@10)\label{tab:noise}}
\centering
\begin{tabular}{l|ccc}
\toprule
\textbf{Method} & $\sigma=0.1$ & $\sigma=0.5$ & $\sigma=1.0$ \\
\midrule
Fixed-Mid & 0.341 & 0.312 & 0.267 \\
\textbf{CARR} & \textbf{0.394} & \textbf{0.371} & \textbf{0.332} \\
\bottomrule
\end{tabular}
\end{table}

CARR maintains a consistent advantage over fixed compression across all noise levels. The gap widens with higher noise ($\sigma=1.0$: $+24.3\%$), suggesting that collapse-aware depth selection also provides a regularising effect against noisy inputs.

\subsubsection{Domain Shift}
We train on ML-1M and evaluate zero-shot on Amazon Beauty (cross-domain transfer). CARR (HR@10: $0.189$) outperforms Fixed-Mid (HR@10: $0.143$) by 32.2\%, indicating that collapse-aware compression learns more transferable latent representations.

\subsection{Experiment 9: Failure Mode Analysis}

We identify conditions under which CARR's benefits are limited.

\subsubsection{Sparse Users}
For cold-start-adjacent users with fewer than 5 interactions, both Fixed-Mid and CARR achieve comparable HR@10 ($0.098$ vs.\ $0.104$, $\Delta < 1\%$). With minimal history, multi-intent geometry is absent and collapse monitoring provides no advantage.

\subsubsection{Single-Intent Users}
For users whose histories are dominated by a single intent cluster (intra-user NMI $< 0.1$), Fixed-Mid and CARR perform comparably (HR@10: $0.385$ vs.\ $0.378$, $\Delta < 2\%$). When intent diversity is negligible, collapse has little impact on recommendation quality.

\subsubsection{Cold-Start Items}
For items with fewer than 10 co-occurrences in training data, collapse-aware compression provides no advantage. Cold-start items lack sufficient covariance structure for the monitoring signal to reliably distinguish safe from unsafe compression depths.

\paragraph{Practical Implications.}
These failure modes delineate the operating envelope of CARR: it delivers largest gains for users with rich, multi-intent histories ($|H| \geq 20$) on datasets with diverse item taxonomies. For sparse, single-intent, or cold-start settings, simpler fixed-compression methods may suffice, and practitioners should assess user-history richness before deploying CARR.

\section{Discussion}

\subsection{Theoretical Implications}
The three theorems jointly establish a coherent theory of reasoning preservation in efficient LLM-based recommendation. Theorem~1 shows that after compression, latent multi-intent geometry becomes progressively more concentrated. Theorem~2 shows that informative historical evidence becomes progressively less influential. Theorem~3 establishes a threshold structure separating safe from collapse regimes. The Jacobian spectral norm analysis (Experiment~1b) provides direct empirical validation of the contractive dynamics assumption underlying all three theorems.
...
\begin{enumerate}
    \item Standard metrics (HR/NDCG) may hide reasoning collapse---monitor $\mathcal{R}$ and $S$ alongside ranking metrics.
    \item Avoid one-size-fits-all compression; adaptive methods like CARR achieve better quality-efficiency trade-offs.
    \item Critical depth varies with data characteristics---sparse datasets require later compression (larger $k^*$).
    \item CARR is most beneficial for users with rich, multi-intent histories; simple compression suffices for sparse users.
\end{enumerate}

\subsection{Limitations}
Our theoretical analysis assumes linearised post-compression dynamics. While the Jacobian analysis confirms approximate contractiveness, the exact critical depth may vary in highly nonlinear regimes or with very large LLMs (e.g., 70B+ parameter models), where full Jacobian estimation is computationally infeasible. Experiments with LLaMA-7B and larger models are left for future work, as are comparisons on the Amazon Reviews 2023 and MIND large-scale datasets. Additionally, computing $\mathcal{R}$ and $S$ during inference adds overhead, though this can be amortised by computing statistics over batches and caching across similar queries.
